...
const periodLabel = computed(() => {
  ...
  const start = dayjs(serviceRequest.value.desired_start_at)
  const end = dayjs(serviceRequest.value.desired_end_at)
    ...
    <template v-else-if="serviceRequest">
      <header class="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <span
          ...
          "
        >
          <CircleCheckBig v-if="serviceRequest.status === 'fulfilled'" :size="15" aria-hidden="true" />
      ...
      <article class="mt-6 rounded-2xl border border-ink/10 bg-surface p-5 shadow-sm sm:p-7">
        ...
        <dl class="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div class="rounded-xl bg-ground p-4">
            ...
            <dd class="mt-2 font-semibold text-ink">{{ formatPrice(serviceRequest.budget_max) }}</dd>
          </div>
          <div class="rounded-xl bg-ground p-4">
            <dt class="flex items-center gap-2 text-xs font-semibold text-ink/45 uppercase">
        ...
        <ul v-else class="mt-6 space-y-3">
          ...
          >
            ...
            <p class="mt-3 flex items-center gap-2 text-sm text-ink/60">
              ...
              {{ dayjs(proposal.scheduled_at).format('DD/MM/YYYY · HH:mm') }}
            </p>
            <p v-if="proposal.message" class="mt-3 rounded-lg bg-ground p-3 text-sm leading-6 text-ink/65">
              {{ proposal.message }}
            </p>
          </li>
        </ul>
